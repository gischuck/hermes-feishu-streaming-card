...
def _render_complete_hook_block(indent: str, newline: str):
    ...
    return [
        ...
        f"{deeper_indent}# Streaming card delivered, but still need to send media files{newline}",
        f"{deeper_indent}if response:{newline}",
        f"{deeper_indent}    _media_adapter = self.adapters.get(source.platform){newline}",
        f"{deeper_indent}    if _media_adapter:{newline}",
        f"{deeper_indent}        await self._deliver_media_from_response(response, event, _media_adapter){newline}",
